...
export const createStore = <T>(fn: Parameters<typeof devtools<T>>[0], name: string) => {
  ...
  const hasReduxDevtools = typeof window !== 'undefined' && '__REDUX_DEVTOOLS_EXTENSION__' in window;

  return create<T>()(devtools(fn, { name, enabled: process.env.NODE_ENV !== 'production' && hasReduxDevtools }));
};
